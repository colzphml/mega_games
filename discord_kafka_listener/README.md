...
```bash
TAG=4.2.0 docker compose build discord-kafka-listener
TAG=4.2.0 docker compose push discord-kafka-listener
```

Обновление на целевом хосте:

```bash
TAG=4.2.0 docker compose pull discord-kafka-listener
TAG=4.2.0 docker compose up -d --no-deps --force-recreate discord-kafka-listener
```
