...
```bash
TAG=4.3.1 docker compose build discord-kafka-week-formatter
TAG=4.3.1 docker compose push discord-kafka-week-formatter
```

Обновление на целевом хосте:

```bash
TAG=4.3.1 docker compose pull discord-kafka-week-formatter
TAG=4.3.1 docker compose up -d --no-deps --force-recreate discord-kafka-week-formatter
```
