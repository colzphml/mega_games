...
```bash
TAG=4.3.1 docker compose build discord-kafka-game-image
TAG=4.3.1 docker compose push discord-kafka-game-image
```

Обновление на целевом хосте:

```bash
TAG=4.3.1 docker compose pull discord-kafka-game-image
TAG=4.3.1 docker compose up -d --no-deps --force-recreate discord-kafka-game-image
```
