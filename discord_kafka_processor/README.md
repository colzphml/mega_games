...
TAG=4.3.1 docker compose push discord-kafka-processor
```

Обновление на целевом хосте:

```bash
TAG=4.3.1 docker compose pull discord-kafka-processor
TAG=4.3.1 docker compose up -d --no-deps --force-recreate discord-kafka-processor
```
