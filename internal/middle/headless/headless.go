...
// NewClient initializes a new headless client with the provided configuration and channels.
...
func (c *Client) ReadMessages(ctx context.Context, wg *sync.WaitGroup) {
	...
	for {
		...
		case message := <-c.MessageChan:
			if err := c.proceedGame(message); err != nil {
...
func (c *Client) proceedGame(message model.DiscordGame) error {
	recapURL, err := recapURLFromGameURL(message.GameUrl)
	if err != nil {
		return err
	}
	recData, err := fetchJSON(recapURL)
	if err != nil {
		return err
	}
	img, err := buildRecapImage(recData)
	if err != nil {
		return err
	}
	...
	if err := png.Encode(&buf, img); err != nil {
		return fmt.Errorf("encode recap image: %w", err)
	}
...
func buildRecapImage(recData Recap) (image.Image, error) {
	// Загружаем фон стадиона и логотипы
	bg, err := loadImage("https://neonsportz.com/images/stadiums/" + strconv.Itoa(recData.Game.HomeTeam.LogoID) + ".png")
	if err != nil {
		return nil, err
	}
	logoHome, err := loadLogo(recData.Game.HomeTeam)
	if err != nil {
		return nil, err
	}
	logoAway, err := loadLogo(recData.Game.AwayTeam)
	if err != nil {
		return nil, err
	}
	footerLogo, err := loadImage("https://neonsportz.com/logo.png")
	if err != nil {
		return nil, err
	}
...
func fetchJSON(url string) (Recap, error) {
...
func loadLogo(t Team) (image.Image, error) {
	if t.Logo != nil {
		return loadImage(*t.Logo)
	}
	return loadImage("https://neonsportz.com/images/teamlogos/256/" + strconv.Itoa(t.LogoID) + ".png")
}
