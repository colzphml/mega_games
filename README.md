...
## Release Workflow (v4.3.0+)

### 1) Build + Push образов (Dev-машина)

```bash
cd /Users/colz/gitrepos/envs/mega_games
TAG=4.3.0 docker compose build
TAG=4.3.0 docker compose push
```
...
```bash
ssh pi 'cd /home/colz/envs/mega_games && TAG=4.3.0 ./scripts/deploy.sh'
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/colzphml/mega_games/v4.3.0/scripts/install.sh \
  | TAG=v4.3.0 INSTALL_DIR=/opt/mega_games bash
```
