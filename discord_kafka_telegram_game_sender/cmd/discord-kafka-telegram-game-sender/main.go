...
func main() {
	...
	healthServer := startHealthServer(ctx, cfg.HealthAddr, storeClient, minioClient, cfg.KafkaBrokers, log)
	defer func() {
...
func startHealthServer(ctx context.Context, addr string, storeClient *store.Store, minioClient *storage.Client, brokers []string, log zerolog.Logger) *http.Server {
