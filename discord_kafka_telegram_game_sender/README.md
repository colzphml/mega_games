...
```bash
TAG=4.3.1 docker compose build discord-kafka-telegram-game-sender
TAG=4.3.1 docker compose push discord-kafka-telegram-game-sender
```

Обновление на целевом хосте:

```bash
TAG=4.3.1 docker compose pull discord-kafka-telegram-game-sender
TAG=4.3.1 docker compose up -d --no-deps --force-recreate discord-kafka-telegram-game-sender
```
