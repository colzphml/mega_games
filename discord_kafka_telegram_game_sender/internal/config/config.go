...
const (
	defaultKafkaConsumerGroup  = "discord-kafka-telegram-game-sender"
	defaultKafkaClientID       = "discord-kafka-telegram-game-sender"
	...
	defaultMinioTimeout        = 5 * time.Second
...
func Load() (Config, error) {
	...
	if cfg.KafkaTopic, err = requiredEnv("KAFKA_GAME_IMAGE_TOPIC"); err != nil {
		return Config{}, err
	}
	cfg.KafkaConsumerGroup = optionalEnv("KAFKA_TELEGRAM_GAME_SENDER_GROUP", defaultKafkaConsumerGroup)
	cfg.KafkaClientID = optionalEnv("KAFKA_TELEGRAM_GAME_SENDER_CLIENT_ID", defaultKafkaClientID)
	...
	if cfg.MinioTimeout, err = durationEnv("MINIO_CONNECT_TIMEOUT", defaultMinioTimeout); err != nil {
		return Config{}, err
	}
	...
	if cfg.TelegramChatID, err = requiredEnv("TELEGRAM_GAME_CHAT_ID"); err != nil {
		return Config{}, err
	}
